...
exports.create = async (req, res, next) => {
  try {
    ...
    const created = await claimRepo.create(req.body);
...
exports.findAll = async (req, res, next) => {
  try {
    if (req.query.policyId) {
      return res.json(await claimRepo.findByPolicyId(req.query.policyId));
    }
    res.json(await claimRepo.findAll());
  } catch (err) {
    next(err);
  }
};
...
exports.findOne = async (req, res, next) => {
  try {
    ...
    if (!record) return res.status(404).json({ error: "NotFound", message: "Claim not found." });
    res.json(record);
  } catch (err) {
    next(err);
  }
...
exports.update = async (req, res, next) => {
  try {
    ...
    if (!existing) return res.status(404).json({ error: "NotFound", message: "Claim not found." });
